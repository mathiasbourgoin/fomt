...
struct AffineSet { u16 pa, pb, pc, pd; };
...
extern "C" void func_0805E99C(DestArray *dest, SrcEntry *src, u32 count, CompositeDesc *desc)
{
    ...
    for (; p != end; p++) {
        ...
        struct AffineSet raw; u32 shape, size, mode; i32 width, height;
        cur.e = *p;
        ...
        if (mode_source == 0) {
            ...
            raw.pa = line[0].affineParam, raw.pb = line[1].affineParam,
            raw.pc = line[2].affineParam, raw.pd = line[3].affineParam;
            *(struct AffineSet *)coeffs.arr = raw;
